...
if __name__ == '__main__':
    ...
    full_test_label = np.load(args.test_label)  # (num_objects, 10000)
    
    print("\n" + "="*60)
    print("Number of Points Robustness Test - Segmentation")
    print("="*60)
    ...
    for num_points in point_counts:
        ...
        # Sample points and labels using nested indices
        test_data = torch.from_numpy(full_test_data[:, indices, :])
        test_label = torch.from_numpy(full_test_label[:, indices])
        ...
        print(f"Points: {num_points:5d} | Accuracy: {test_accuracy:.4f} | Drop: {accuracy_drop:.4f}")

        # Save visualization if requested
        if args.visualize:
            viz_dir = os.path.join(args.output_dir, 'num_points_viz')
            create_dir(viz_dir)
            
            # Visualize ground truth (only once at 10000 points)
            if num_points == 10000:
                viz_seg(
                    test_data[args.viz_idx], 
                    test_label[args.viz_idx], 
                    os.path.join(viz_dir, f"gt_{num_points}pts.gif"), 
                    args.device
                )
            ...
            viz_seg(
                ...
                os.path.join(viz_dir, f"pred_{num_points}pts.gif"), 
                args.device
            )

    # Save results to CSV
    csv_path = os.path.join(args.output_dir, 'num_points_results_seg.csv')
    ...
    if args.visualize:
        print(f"Visualizations saved to: {os.path.join(args.output_dir, 'num_points_viz')}")
    print("="*60)
    print("\nSummary:")
